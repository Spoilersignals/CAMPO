...
import { z } from "zod";
import { Upload, X, DollarSign, Info } from "lucide-react";
...
interface ListingFormProps {
  ...
  onSubmit: (data: ListingFormValues, photos: File[]) => Promise<void>;
...
export function ListingForm({
  ...
  const [photosPreviews, setPhotosPreviews] = React.useState<string[]>(existingImages);
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="font-medium text-blue-900">Commission Fee</h4>
                <p className="mt-1 text-sm text-blue-700">
                  A {COMMISSION_RATE * 100}% commission of{" "}
                  <span className="font-semibold">${commission.toFixed(2)}</span> will be
                  required before your listing goes live.
                </p>
                <div className="mt-2 flex items-center gap-1 text-xs text-blue-600">
                  <Info className="h-3 w-3" />
                  <span>You&apos;ll pay this after submitting your listing</span>
                </div>
              </div>
            </div>
          </CardContent>
      ...
      <Button type="submit" disabled={isSubmitting} className="w-full">
        {isSubmitting ? "Submitting..." : defaultValues ? "Update Listing" : "Create Listing"}
      </Button>
    </form>
  );
}
